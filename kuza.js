...
const { genshin } = require('./listsell/genshin')
...
const simple = require('./lib/simple.js')
    ...
    let bca = fs.readFileSync("./payment/QRIS-3.jpg");

    //--Setting--//

let botname = '*_｢ Azura Bot ｣ 冬_*' //NAMA BOT
let lolkey = 'isi'
...
let ownername = '*_｢ Azura ｣_*' //NAMA OWNER
let owner = '6285701662971' // NOMOR OWNER
let symbol = '*🦅*'
let faketeks = `*_｢ Azura Bot ｣ 冬_*` 
...
const ytabilbotz = "6285701662971@s.whatsapp.net"
const txt = mek.message.conversation
const botNumber = kuza.user.jid
const ownerNumber = [`${owner}@s.whatsapp.net`, `6285701662971@s.whatsapp.net`, `6285701662971@s.whatsapp.net`]
...
const katalog = (teks) => {
res = kuza.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 1, "message": teks, "footerText": "Azura Bot", "thumbnail": fs.readFileSync('./media/pee.jpg'), "surface": 'CATALOG' }}, {quoted:fstatus})
kuza.relayWAMessage(res)}
const fakeyt = (teks) => { 
kuza.sendMessage(from, teks, text,{contextInfo :{text: 'hi', "forwardingScore": 1000000000, isForwarded: false, sendEphemeral: false, "externalAdReply": { "title": `hallo ${pushname}🗿` , "body": `🌀 _Created by Azura Bot_`, "mediaType": "2", "thumbnailUrl": "https://c.top4top.io/p_2087f30hj1.jpeg", "mediaUrl": "https://tps.com/channel/UCJPqI5eVhKPXPL2V8y6pIDA", "thumbnail": fs.readFileSync('./media/pee.jpg'), "sourceUrl": "", },mentionedJid:[sender]}, quoted : fstatus})};
...
const fstatus = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "🌀 _Created by Azura Bot_", 'jpegThumbnail': fs.readFileSync('./media/pee.jpg')}}}
...
run = process.uptime();
 ...
 footerText: '🌀 _Created by Azura Bot_',
 description: `
 *_｢ Indonesia Time ｣_*
${symbol} Date : ${date}
${symbol} Wib : ${Wib}
...
Hai kak @${sender.split('@')[0]}, Silahkan pilih menu di bawah
`,
 sections: [
                     {
...
run = process.uptime();
 listMsg = {
 buttonText: 'MENU',
 footerText: '🌀 _Created by Azura Bot',
 description: `
...
Hai kak @${sender.split('@')[0]}, Silahkan Pilih Menu Di Bawah Ya
`,
 sections: [
                     {
                      "title": `Pilih Salah Satu`,
...
${dmff}`
  but = [
{ buttonId: `.owner`, buttonText: { displayText: 'Admin' }, type: 1 },
{ buttonId: `.id`, buttonText: { displayText: 'Format Pemesanan' }, type: 1 },
...
run = process.uptime();
 ...
 footerText: '🌀 _Created by Azura Bot_',
 description: `
Hai kak @${sender.split('@')[0]}, Silahkan Pilih Pembayaran Di Bawah Ya
`,
 sections: [
                     {
                      "title": `Pilih Salah Satu`,
...
*Nomor Gopay : 085701662971*
*A/n Agung Yudisaputro*`
teks = `*NOTE* : 
- JANGAN LUPA KIRIM BUKTI TRANSFER
- QRIS HANYA UNTUK DANA
- JIKA SUDAH SCAN QRIS SELAIN DANA = AKAN DI REFUND SETELAH 24 JAM`
but = [
...
*_Silahkan Mengirimkan Data Ke Grup Beserta Bukti Transfernya_*
*_Terima Kasih_*`)
break
...
costum(buffer, sticker, Verived, `Azura Bot`)
fs.unlinkSync(media)
fs.unlinkSync(ran)
})
