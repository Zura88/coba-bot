...
const starts = async (kuza = new WAConnection()) => {
    ...
    console.log(banner)
console.log(color('🌀[ BOT BY AZURA ]'))
    kuza.on('qr', () => {
...
kuza.on("group-participants-update", async (anu) => {
			      ...
			      if (anu.action == "add" && mem.includes(kuza.user.jid)) {
			        kuza.sendMessage(anu.jid, "Bot Active ketik #menu untuk menampilkan Menu Bot", "conversation")}
                if (!isWelkom) return
                if (anu.action == "add" && mem.includes(kuza.user.jid)) {
                kuza.sendMessage(anu.jid, "Bot Active ketik #menu untuk menampilkan Menu Bot", "conversation")}
                if (anu.action == 'add' && !mem.includes(kuza.user.jid)) {
...
function nocache(module, cb = () => { }) {
    console.log(color('Module', 'green'), color(`'${module} Dipantau Oleh Azura'`, 'cyan'))
